  ...
  return (
    <SheetPortal>
      ...
      <SheetPrimitive.Content
        ...
        className={cn(
          // Base styles
          "bg-background fixed z-50 flex flex-col bg-clip-padding text-sm shadow-lg transition duration-200 ease-in-out",
          // Animations
          ...
          // Max width for side panels
          "data-[side=left]:sm:max-w-md data-[side=right]:sm:max-w-md",
          // ✅ Overflow hidden pour contenir le scroll interne
          "overflow-hidden",
          className
        )}
        {...props}
...
function SheetHeader({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn(
        "flex flex-col gap-1.5 p-4 pb-4 flex-shrink-0 border-b",
        className
      )}
      {...props}
    />
...
/**
 * SheetBody — Zone de contenu scrollable
 * Utilise cette zone pour le contenu principal du drawer
 * Ajoute `space-y-6` pour espacer les sections
 */
...
function SheetFooter({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn(
        "flex flex-col gap-2 p-4 pt-4 flex-shrink-0 border-t",
        className
      )}
      {...props}
    />
